...
int 
RoutingHeader::GetSize()
{
  return 6; // src:2  dst:2
}
...
void 
RoutingHeader::SetSA(AquaSimAddress sa)
{
  m_sa = sa;
}
void 
RoutingHeader::SetDA(AquaSimAddress da)
{
  m_da = da;
}
void 
RoutingHeader::SetNumForwards(uint16_t numForwards)
{
...
{
  os << "Dynamic Routing Header is: PktSrc=" << m_pktSrc <<" PktDst="<<
    m_pktDst<< " numForward="<<numForward<<" PktLen=" <<
...
DRoutingHeader::SetPktSrc(AquaSimAddress pktSrc)
{
 ...
 void 
 DRoutingHeader::SetPktDst(AquaSimAddress pktDst)
 {
   m_pktDst = pktDst;
 }
 void
 DRoutingHeader::SetNumForwards(uint16_t number)
 {
   numForward = number;
 }
void
...
void
DRoutingHeader::SetPktSeqNum(uint16_t pktSeqNum)
{
...
void
VBHeader::SetNumForwards(uint16_t numforwards)
{
  m_numForwards = numforwards;
}
void
...
void
DBRHeader::SetNHops(uint16_t hops)
{
  m_nhops = hops;
}
void
...
void
DBRHeader::SetTarget(AquaSimAddress target)
{
  m_target = target;
}
void
DBRHeader::SetNumForward(uint16_t numforward)
{
