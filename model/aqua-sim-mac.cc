 ...
 */
 ...
 // chenhao remove getPremble on Mar 11 2022
...
TypeId AquaSimMac::GetTypeId(void) {
	...
	.AddAttribute("SetNetDevice", "A pointer to connect to the net device.",
			PointerValue(), MakePointerAccessor(&AquaSimMac::m_device),
			MakePointerChecker<AquaSimMac>()).AddAttribute("BitRate",
			"Bit rate of MAC layer.", DoubleValue(1e4),
			MakeDoubleAccessor(&AquaSimMac::m_bitRate),
			MakeDoubleChecker<double>()).AddAttribute("EncodingEfficiency",
			"Ratio of encoding", DoubleValue(1),
	...
	.AddTraceSource("RoutingTx",
			...
			"ns3::AquaSimMac::TxCallback").AddTraceSource("RoutingRx",
