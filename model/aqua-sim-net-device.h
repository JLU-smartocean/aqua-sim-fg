...
public:
	...
	void SetMac(Ptr<AquaSimMac> mac, Ptr<AquaSimSync> sync = NULL,
			Ptr<AquaSimLocalization> loc = NULL);
	...
	int GetFrequency();
	void SetValuesCompare(double v);
	double GetValuesCompare();
	void SetClusterSend(int c);
	int GetClusterSend();
	...
	// void SetNodePosition(int num,int x,int y);
//  std::vector<Position>  GetNodePosition();
	void PushMember(int member);
	std::vector<int> GetMember();
//  std::vector <Position> pos;
	virtual void DoDispose(void);
	virtual void DoInitialize(void);

	void ForwardUp(Ptr<Packet> packet, Ptr<MobilityModel> src,
			Ptr<MobilityModel> dst);	//not used.
	void SendUp(Ptr<Packet> p);
	...
	inline Time& PositionUpdateTime(void) {
		return m_positionUpdateTime;
	}
	void SetPositionUpdateTime(Time posUpdateTime) {
		m_positionUpdateTime = posUpdateTime;
	}

	//sink related attributes
	int ClearSinkStatus(void);
	int SetSinkStatus(void);
	inline int GetSinkStatus(void) {
		return m_sinkStatus;
	}

	//VBF
	inline double& CX(void) {
		return m_cX;
	}
	inline double& CY(void) {
		return m_cY;
	}
	inline double& CZ(void) {
		return m_cZ;
	}

	inline bool FailureStatus(void) {
		return m_failureStatus;
	}
	inline double FailurePro(void) {
		return m_failurePro;
	}
	inline double FailureStatusPro(void) {
		return m_failureStatusPro;
	}

	void SetTransmissionStatus(TransStatus status);
	TransStatus GetTransmissionStatus(void);

	inline bool CarrierSense(void) {
		return m_carrierSense;
	}
	inline void ResetCarrierSense(void) {
	...
	inline bool CarrierId(void) {
		return m_carrierId;
	}
	inline void ResetCarrierId(void) {
		m_carrierId = false;
	}
	...
	Ptr<AquaSimEnergyModel> EnergyModel(void) {
		return m_energyModel;
	}
	bool IsAttacker(void);

	int TotalSentPkts() {
		return m_totalSentPkts;
	}
protected:
...
private:
	...
	int clusterhead = 0;
	// Position pointposition;//Each round of initialization should be cleared
