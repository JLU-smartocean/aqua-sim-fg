...
MacHeader::MacHeader() : m_demuxPType(UWPTYPE_OTHER)
{
}
...
void MacHeader::SetSA(AquaSimAddress sa)
{
	m_sa = sa;
}
void MacHeader::SetDA(AquaSimAddress da)
{
	m_da = da;
}
void MacHeader::SetDemuxPType(uint8_t demuxPType)
{
...
void TMacHeader::SetPtype(uint8_t ptype)
{
	m_ptype = ptype;
}
void TMacHeader::SetPktNum(uint32_t pktNum)
{
	m_pktNum = pktNum;
}
void TMacHeader::SetDataNum(uint32_t dataNum)
{
	m_dataNum = dataNum;
}
void TMacHeader::SetBlockNum(uint8_t blockNum)
{
	m_blockNum = blockNum;
}
void TMacHeader::SetSenderAddr(AquaSimAddress senderAddr)
{
	m_senderAddr = senderAddr;
}
void TMacHeader::SetRecvAddr(AquaSimAddress recvAddr)
{
	m_recvAddr = recvAddr;
}
void TMacHeader::SetST(double st)
{
	m_st = st;
}
void TMacHeader::SetTS(double ts)
{
	m_ts = ts;
}
void TMacHeader::SetDuration(double durable)
{
	m_duration = durable;
}
void TMacHeader::SetInterval(double interval)
{
	m_interval = interval;
}
void TMacHeader::SetArrivalTime(double arrivalTime)
{
...
void TMacHeader::Serialize(Buffer::Iterator start) const
{
...
void TMacHeader::Print(std::ostream &os) const
{
...
std::vector<std::string> AlohaHeader::pktTypeName = {
	"DATA", "ACK"
};
...
void AlohaHeader::setPktType(uint8_t pktType)
{
	this->pktType = pktType;
}

uint8_t
AlohaHeader::getPktType()
{
	return pktType;
}

void AlohaHeader::setPktSeq(uint8_t pktSeq)
{
	this->pktSeq = pktSeq;
}

uint8_t
AlohaHeader::getPktSeq()
{
	return pktSeq;
}
...
void AlohaHeader::Serialize(Buffer::Iterator start) const
{
	MacHeader::Serialize(start);

    start.Next(MacHeader::GetSerializedSize());

    start.WriteU8(pktType);
    start.WriteU8(pktSeq);
}
...
void AlohaHeader::Print(std::ostream &os) const
{
	const MacHeader *macHeader = this;

	macHeader->Print(os);
	os << pktTypeName[pktType];
	os << "\n";
}
...
void FamaHeader::SetSA(AquaSimAddress sa)
{
	SA = sa;
}
void FamaHeader::SetDA(AquaSimAddress da)
{
	DA = da;
}
void FamaHeader::SetPType(uint8_t pType)
{
	m_pType = pType;
}
AquaSimAddress
...
void FamaHeader::Serialize(Buffer::Iterator start) const
{
...
void FamaHeader::Print(std::ostream &os) const
{
...
void CopeHeader::SetSA(AquaSimAddress sa)
{
	SA = sa;
}
void CopeHeader::SetDA(AquaSimAddress da)
{
	DA = da;
}
void CopeHeader::SetPType(uint8_t pType)
{
	m_pType = pType;
}
AquaSimAddress
...
void CopeHeader::Serialize(Buffer::Iterator start) const
{
	start.WriteU16(SA.GetAsInt());
	start.WriteU16(DA.GetAsInt());
	//start.WriteU8 (SA.GetLength());
...
void CopeHeader::Print(std::ostream &os) const
{
...
std::vector<std::string> SFamaHeader::pktTypeName = {
	"RTS", "CTS", "DATA", "ACK"
};
...
void SFamaHeader::setPktType(uint8_t pktType)
{
	this->pktType = pktType;
}

void SFamaHeader::setSlotNum(uint16_t slotNum)
{
	this->slotNum = slotNum;
}

uint8_t
SFamaHeader::getPktType()
{
	return pktType;
}
...
{
	return 1 + 2 + MacHeader::GetSerializedSize();
}

void SFamaHeader::Serialize(Buffer::Iterator start) const
{
	MacHeader::Serialize(start);
...
void SFamaHeader::Print(std::ostream &os) const
{
	const MacHeader *macHeader = this;
...
int UwanSyncHeader::GetSize()
{
	return 8 * sizeof(Time);
}
void UwanSyncHeader::SetCyclePeriod(double cyclePeriod)
{
...
void UwanSyncHeader::Serialize(Buffer::Iterator start) const
{
...
void UwanSyncHeader::Print(std::ostream &os) const
{
...
void LocalizationHeader::SetNodePosition(Vector nodePosition)
{
...
void LocalizationHeader::Serialize(Buffer::Iterator start) const
{
	Buffer::Iterator i = start;
...
void LocalizationHeader::Print(std::ostream &os) const
{
	os << "Localization Header: nodePosition(" << m_nodePosition.x << "," << m_nodePosition.y << "," << m_nodePosition.z << "), confidence=" << m_confidence << "\n";
}
TypeId
...
TypeId
PhyFamaHeader::GetTypeId()
{
	static TypeId tid = TypeId("ns3::PhyFamaHeader")
							.SetParent<Header>()
							.AddConstructor<PhyFamaHeader>();
	return tid;
}

int PhyFamaHeader::size()
{
	return sizeof(AquaSimAddress) * 4 + 1; /*for packet_type*/
}
void PhyFamaHeader::SetSA(AquaSimAddress sa)
{
	SA = sa;
}
void PhyFamaHeader::SetDA(AquaSimAddress da)
{
	DA = da;
}
void PhyFamaHeader::SetPType(uint8_t pType)
{
	m_pType = pType;
}
AquaSimAddress
...
uint8_t
PhyFamaHeader::GetPType()
{
	return m_pType;
}

uint32_t
PhyFamaHeader::GetSerializedSize(void) const
{
	return 2 + 2 + 1;
}
void PhyFamaHeader::Serialize(Buffer::Iterator start) const
{
	start.WriteU16(SA.GetAsInt());
	start.WriteU16(DA.GetAsInt());
	//start.WriteU8 (SA.GetLength());
...
uint32_t
PhyFamaHeader::Deserialize(Buffer::Iterator start)
{
	Buffer::Iterator i = start;
	SA = (AquaSimAddress)i.ReadU16();
	DA = (AquaSimAddress)i.ReadU16();
...
void PhyFamaHeader::Print(std::ostream &os) const
{
	os << "FAMA Header: SendAddress=" << SA << ", DestAddress=" << DA << ", PacketType=";
	switch (m_pType)
	{
...
TypeId
PhyFamaHeader::GetInstanceTypeId(void) const
{
	return GetTypeId();
}
