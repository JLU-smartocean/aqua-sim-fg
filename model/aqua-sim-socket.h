 ...
 */
#ifndef AQUA_SIM_SOCKET_H
#define AQUA_SIM_SOCKET_H
...
class AquaSimSocket : public Socket
{
public:
  /**
  ...
  virtual void ForwardUp (Ptr<NetDevice> device, Ptr<const Packet> packet,
...
private:
  ...
  int DoBind (const AquaSimSocketAddress &address);
  ...
  uint32_t GetMinMtu (AquaSimSocketAddress ad) const;
  virtual void DoDispose (void);

  /**
   * subpackage and mergepacket function
   * add by Spinach
   */
  int SubPacket(Ptr<Packet> pkt, uint8_t pktMode);
  Ptr<Packet> RecvPktProcess(Ptr<const Packet> packet, AquaSimSocketAddress & address);
  Ptr<Packet> MergePacket(std::queue<Ptr<Packet> > & PktRecvQue);
  std::queue<Ptr<Packet>> PktSendQue;
  // std::queue<Ptr<Packet>> PktRecvQue;
  uint32_t                packetcount;
  std::map<std::pair<Address, uint8_t>, uint8_t> recvCounts;
  std::map<std::pair<Address, uint8_t>, std::queue<Ptr<Packet> > > PktRecvQues;
 ...
 */
class AquaSimSocketTag : public Tag
{
public:
  /**
 ...
 */
class AquaDeviceNameTag : public Tag
{
public:
  /**
